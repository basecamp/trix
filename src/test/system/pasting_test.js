...
import { TEST_IMAGE_URL, after, assert, clickToolbarButton, createFile, defer,
  expandSelection, moveCursor, pasteContent, pressKey, test, testGroup, testIf,
  triggerEvent, typeCharacters } from "test/test_helper"

testGroup("Pasting", { template: "editor_empty" }, function() {
  test("paste plain text", expectDocument => typeCharacters("abc", () => moveCursor("left", () => pasteContent("text/plain", "!", () => expectDocument("ab!c\n")))))

  test("paste simple html", expectDocument => typeCharacters("abc", () => moveCursor("left", () => pasteContent("text/html", "&lt;", () => expectDocument("ab<c\n")))))

  test("paste complex html", expectDocument => typeCharacters("abc", () => moveCursor("left", () => pasteContent("text/html", "<div>Hello world<br></div><div>This is a test</div>", () => expectDocument("abHello world\nThis is a test\nc\n")))))

  test("paste html in expanded selection", expectDocument => typeCharacters("abc", () => moveCursor("left", () => expandSelection({ direction: "left", times: 2 }, () => pasteContent("text/html", "<strong>x</strong>", function() {
  ...
  test("paste html with CRLF ", expectDocument => pasteContent("text/html", "<div>a<br></div>\r\n<div>b<br></div>\r\n<div>c<br></div>", () => expectDocument("a\nb\nc\n")))

  test("paste unsafe html", function(done) {
    window.unsanitized = []
    const pasteData = {
      "text/plain": "x",
      "text/html": `\
...
`
    ...
    return pasteContent(pasteData, () => after(20, function() {
  ...
  test("paste URL", expectDocument => typeCharacters("a", () => pasteContent("URL", "http://example.com", function() {
  ...
  test("paste complex html into formatted block", done => typeCharacters("abc", () => clickToolbarButton({ attribute: "quote" }, () => pasteContent("text/html", "<div>Hello world<br></div><pre>This is a test</pre>", function() {
  ...
  test("paste list into list", done => clickToolbarButton({ attribute: "bullet" }, () => typeCharacters("abc\n", () => pasteContent("text/html", "<ul><li>one</li><li>two</li></ul>", function() {
    const document = getDocument()
  ...
  test("paste list into quote", done => clickToolbarButton({ attribute: "quote" }, () => typeCharacters("abc", () => pasteContent("text/html", "<ul><li>one</li><li>two</li></ul>", function() {
    const document = getDocument()
    assert.equal(document.getBlockCount(), 3)
  ...
  test("paste list into quoted list", done => clickToolbarButton({ attribute: "quote" }, () => clickToolbarButton({ attribute: "bullet" }, () => typeCharacters("abc\n", () => pasteContent("text/html", "<ul><li>one</li><li>two</li></ul>", function() {
    const document = getDocument()
    assert.equal(document.getBlockCount(), 3)
  ...
  test("paste nested list into empty list item", done => clickToolbarButton({ attribute: "bullet" }, () => typeCharacters("y\nzz", function() {
    getSelectionManager().setLocationRange({ index: 0, offset: 1 })
    return defer(() => pressKey("backspace", function() {
  ...
  test("paste nested list over list item contents", done => clickToolbarButton({ attribute: "bullet" }, () => typeCharacters("y\nzz", function() {
    getSelectionManager().setLocationRange({ index: 0, offset: 1 })
    return defer(() => expandSelection("left", function() {
      pasteContent("text/html", "<ul><li>a<ul><li>b</li></ul></li></ul>", function() {})
      const document = getDocument()
      assert.equal(document.getBlockCount(), 3)
  ...
  test("paste list into empty block before list", done => clickToolbarButton({ attribute: "bullet" }, () => typeCharacters("c", () => moveCursor("left", () => pressKey("return", function() {
    getSelectionManager().setLocationRange({ index: 0, offset: 0 })
    return defer(() => pasteContent("text/html", "<ul><li>a</li><li>b</li></ul>", function() {
      const document = getDocument()
      assert.equal(document.getBlockCount(), 3)
  ...
  test("paste file", expectDocument => typeCharacters("a", () => pasteContent("Files", createFile(), () => expectDocument(`a${OBJECT_REPLACEMENT_CHARACTER}\n`))))

  return testIf(config.input.getLevel() === 0, "paste event with no clipboardData", expectDocument => typeCharacters("a", function() {
